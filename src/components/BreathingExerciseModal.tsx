...
const TECHNIQUES: Record<Technique, { name: string; description: string; phases: PhaseConfig[] }> = {
  ...
  '478': {
    ...
    description: 'Deep autonomic relaxation to soothe anxiety and invite sleep.',
    phases: [
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full border border-emerald-100 dark:border-emerald-950/80 shadow-2xl p-6 sm:p-8 relative overflow-hidden flex flex-col items-center text-center"
      >
        <button
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl mb-4 text-xs font-bold">
          <button
        ...
        <div className="relative w-56 h-56 flex items-center justify-center my-4">
          <div
            className="absolute inset-0 rounded-full bg-emerald-100/40 dark:bg-emerald-950/30 border border-emerald-200/60 dark:border-emerald-800/40"
            style={{
          ...
          />
          <div
            className="w-36 h-36 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-400 dark:from-emerald-600 dark:to-teal-500 text-white shadow-lg flex flex-col items-center justify-center relative z-10"
            style={{
        ...
        <div className="min-h-[44px] flex items-center justify-center mb-6">
          <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 px-4">
            {activePhase.instruction}
          </p>
        </div>

        {/* Controls */}
        <div className="flex items-center justify-center gap-4 w-full pt-4 border-t border-slate-100 dark:border-slate-800">
          <button
            id="breathing-reset-btn"
            onClick={handleReset}
            className="p-3 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            title="Restart cycle"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold shadow-md shadow-emerald-600/20 transition-all active:scale-95 text-xs sm:text-sm"
          >
            {isRunning ? (
              <>
          ...
          <div className="text-xs text-slate-500 dark:text-slate-400 font-bold px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-xl">
